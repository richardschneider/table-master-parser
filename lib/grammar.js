...
g.addRule('handPossessive', p.either(
...
g.addRule('vulnerable', p.either(
    p.keyword('Neither vulnerable').op(push('Neither vulnerable')),
    p.keyword('N/S vulnerable').op(push('N/S vulnerable')),
    p.keyword('E/W vulnerable').op(push('E/W vulnerable')),
    p.keyword('Both vulnerable').op(push('Borth vulnerable'))
    )
);

g.addRule('card', p.either(
    p.keyword('AS').op(push('AS')) // TODO: add remaining cards
    )
);
...
messages.add('play',
    p.sequence(p.other('hand'), p.keyword('plays'), p.other('card'))
    .op(ast('play', 'seat', 'card'))
);

messages.add('readyForTeams',
    p.sequence(p.other('hand'), p.keyword('ready'), p.keyword('for'), p.keyword('teams'))
    .op(ast('readyForTeams', 'seat'))
);

messages.add('readyToStart',
    p.sequence(p.other('hand'), p.keyword('to'), p.keyword('start'))
...
messages.add('readyForDeal',
    p.sequence(p.other('hand'), p.keyword('ready'), p.keyword('for'), p.keyword('deal'))
    .op(ast('readyForDeal', 'seat'))
);
messages.add('readyForCards',
    p.sequence(p.other('hand'), p.keyword('ready'), p.keyword('for'), p.keyword('cards'))
    .op(ast('readyForCards', 'seat'))
);
messages.add('deal',
    p.sequence(
        p.keyword('board'), p.number(), p.keyword('.'),
        p.keyword('dealer'), p.other('hand'), p.keyword('.'),
        p.other('vulnerable'))
    .op(ast('deal', 'board', 'dealer', 'vulnerable'))
);
messages.add('cards',
    p.sequence(
        p.other('handPossessive'),
        p.keyword('cards'),
        p.keyword(':'),
        p.other('playersHand'))
...
messages.add('bid',
    p.sequence(p.other('hand'), p.other('call'))
    .op(ast('bid', 'seat', 'bid'))
);
